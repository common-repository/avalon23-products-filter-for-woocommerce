...
class DataTable23 {
    constructor(table_data, table_html_id, additional = {}) {
        ...
        let defaults = {
            mode: 'ajax', //ajax,json
            ajax_url: avalon23_helper_vars.ajax_url,
            heads: {title: 'Title'},
            cells_width: [],
            hide_on_mobile: [],
            orders: {title: false},
            ...
            use_flow_header: false,
            lang: {
                loading: avalon23_helper_vars.lang.loading,
                no_data: avalon23_helper_vars.lang.no_data,
                error: avalon23_helper_vars.lang.error,
                items: avalon23_helper_vars.lang.items,
                page: avalon23_helper_vars.lang.page,
                pages: avalon23_helper_vars.lang.pages
            },
            pagination: {
                position: 't', //t,b,tb
                next: {
                    class: 'btn',
                    content: avalon23_helper_vars.lang.next
                },
                prev: {
                    class: 'btn',
                    content: avalon23_helper_vars.lang.prev
                },
                input: {
                    class: 'form-control'
                }
        ...
        //lang fix
        this.request_data.lang = avalon23_helper_vars.selected_lang;
        ...
        this.table.setAttribute('id', avalon23_helper.create_id('t'));
        ...
        if (this.settings.use_flow_header) {
        ...
        if (this.container.querySelector('.avalon23-place-loader')) {
            this.container.querySelector('.avalon23-place-loader').remove();
        }
        ...
        if (this.request_data.mode == 'json' || this.settings.use_load_more) {
        ...
        if (this.get_cols_count() > 0) {
            ...
            Object.keys(this.settings.heads).forEach((field_key) => {
                ...
                if (this.is_mobile) {
                    if (this.settings.hide_on_mobile.includes(field_key) || this.settings.hide_on_mobile.includes(field_key.replace('t:', ''))) {
                        return false;
                    }
                }
                ...
                if (this.settings.orders[field_key]) {
                    th.classList.add('order-able');
                    if (this.request_data.orderby == field_key) {
                        th.classList.add('order-' + this.request_data.order);
                    }
                }
            ...
            var th_order_able = this.table.querySelectorAll('th.order-able');
            ...
            if (th_order_able.length > 0) {
                for (let i = 0; i < th_order_able.length; i++) {
                    th_order_able[i].addEventListener('click', function (e) {
                        ...
                        if (this.classList.contains('order-desc')) {
                        ...
                        _this.table.querySelectorAll("th[data-key='" + _this.request_data.orderby + "']").forEach(function (item) {
                            item.classList.add('order-' + _this.request_data.order);
                        });
        ...
        document.addEventListener('avalon23-table-reset-cache', e => {
        ...
        document.addEventListener('avalon23-table-get', e => {
            if (this.table_html_id === e.detail.table_html_id) {
            ...
            window.addEventListener('resize', (e) => {
                if (!this.resize_ev_is_locked) {
                    ...
                    if (this.show_compact) {
                        this.use_flow_header = false;
                        this.flow_header.style.display = 'none';
                        this.container.classList.add('avalon23-in-compact-mode');
                    } else {
                        this.container.classList.remove('avalon23-in-compact-mode');
                    }
...
//console.log(body);
    ...
    build_rows(data) {
        ...
        for (let x = 0; x < data.length; x++) {
            ...
            Object.keys(data[x]).forEach(function (current_field) {
                ...
                if (_this.is_mobile) {
                    if (_this.settings.hide_on_mobile.includes(current_field) || _this.settings.hide_on_mobile.includes(current_field.replace('t:', ''))) {
                        return;
                    }
                }
                ...
                td.className = 'avalon23_td_cell avalon23_td_' + current_field.replace(':', '') + css_class;
    ...
    __draw_print_button() {
        if (typeof this.print_btn_created === 'undefined') {
            if (parseInt(this.settings.show_print_btn)) {
                ...
                btn.className = 'table23-print-btn avalon23-btn';
                ...
                btn.addEventListener('click', (e) => {
                    ...
                    if (typeof avalon23_print_window_width === 'undefined') {
                        var avalon23_print_window_width = 800;
                    }

                    if (typeof avalon23_print_window_height === 'undefined') {
                        var avalon23_print_window_height = 600;
                    }

                    let mywindow = window.open('', 'PRINT', `width=${avalon23_print_window_width},height=${avalon23_print_window_height}`);
                    mywindow.document.write('<html><head><title>' + document.title + '</title>');

                    mywindow.document.write(`<link rel="stylesheet" href="${avalon23_helper_vars.assets_url}js/data-table-23/print.css" type="text/css" />`);
                    ...
                    let do_not_print = ['variations', 'grouped', 'file'];

                    if (typeof avalon23_do_not_print !== 'undefined') {
                        do_not_print = [...do_not_print, ...avalon23_do_not_print];
                    }

                    Loop1:
                            for (var i = 0; i < do_not_print.length; i++) {
                        Loop2:
                                for (var th of mywindow.document.getElementsByTagName('th')) {
                            if (th.getAttribute('data-key') === do_not_print[i]) {
                                th.remove();
                            }
                        }
                    }
                    ...
                    Loop1:
                        ...
                        Loop2:
                            ...
                            if (td.getAttribute('data-field').trim() === do_not_print[i].trim()) {
                                td.remove();
                            }
                        }
                    }
        ...
        if (pages_count > 1) {
            ...
            for (var p = 0; p < buttons.length; p++) {
                ...
                if (buttons[p] === 0) {
                    ...
                    input.className = 'avalon23-data-table-pager';
                    ...
                    input.addEventListener('change', function (e) {
                        ...
                        _this.wrapper.parentElement.querySelectorAll('.avalon23-data-table-current-page span').forEach(function (item) {
                        ...
                        _this.wrapper.parentElement.querySelector('.table23-pagination .avalon23-data-table-prev').classList.remove('table23-hidden');
                        _this.wrapper.parentElement.querySelector('.table23-pagination .avalon23-data-table-next').classList.remove('table23-hidden');

                        if (_this.request_data.current_page <= 0) {
                            _this.request_data.current_page = 0;

                            _this.wrapper.parentElement.querySelectorAll('.avalon23-data-table-prev').forEach(function (item) {
                                item.classList.add('table23-hidden');
                            });
                            _this.wrapper.parentElement.querySelectorAll('.avalon23-data-table-next').forEach(function (item) {
                                item.classList.remove('table23-hidden');
                            });
                        }
                ...
                if (buttons[p] > 0) {
                    ...
                    a.classList.add('avalon23-data-table-next');
                    ...
                    a.classList.add('avalon23-data-table-prev');
                ...
                a.addEventListener('click', function (e) {
                    ...
                    _this.wrapper.parentElement.querySelectorAll('.avalon23-data-table-pager').forEach(function (item) {
            ...
            var li = document.createElement('span');
            li.className = 'avalon23-data-table-current-page';
    ...
    draw_load_more_button() {
        ...
        button.className = 'avalon23-btn';
        button.innerHTML = avalon23_helper_vars.lang.load_more;
    ...
    init_json_fields_saving() {
        ...
        document.addEventListener('after_' + this.save_filter_field_action, this.change_cell_ev_handler);
    }

    init_json_fields_saving_listener(e) {
        if (this.settings.mode === 'json'/* && e.detail.self === this*/) {
            //console.log('event ' + 'after_' + this.save_filter_field_action);

            let cell = this.get_cell(e.detail.posted_id, e.detail.field);
            ...
            if (cell) {
                ...
                this.save_jsoned_field(e.detail.field, cell.innerHTML, e.detail.posted_id);
            }
        }
    }

    //saving cell data for json-mode tables
    save_jsoned_field(field, value, posted_id) {

        if (this.settings.mode === 'json') {
            if (typeof this.settings.table_data.rows !== 'undefined') {
                for (const key of Object.keys(this.settings.table_data.rows)) {
                    if (parseInt(posted_id) === parseInt(this.settings.table_data.rows[key].pid)) {
                ...
                for (const key of Object.keys(this.settings.table_data)) {
                    if (parseInt(posted_id) === parseInt(this.settings.table_data[key].pid)) {
